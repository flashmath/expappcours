/**
 * Created by Fabrice on 06/03/2016.
 */
(function() {
    var indexOf = [].indexOf || function(item) { for (var i = 0, l = this.length; i < l; i++) { if (i in this && this[i] === item) return i; } return -1; };

    angular.module('timezoneSelect', []).directive('timezoneSelect', function() {
        var allTimes;
        allTimes = [
            {"group":"US (Common)",
                "zones":[
        ...
        return {
            ...
            template: '<select><optgroup ng-repeat="time in times" label="{{time.group}}">' +
            '<option ng-repeat="zone in time.zones" value="{{zone.value}}">{{zone.name}}</option>' +
            '</optgroup></select>',
                //'<select ng-options="country.code as country.name for country in countries"> <option value="" ng-if="isSelectionOptional"></option> </select>',
            controller: [
                '$scope', '$attrs', function($scope, $attrs) {
                    var timeCodesIn, findTimesIn, includeOnlyRequestedTimes, removeTime, removeExcludedTimes, separator, updateWithPriorityTimes;
                    separator = {
                        code: '-',
                        name: '────────────────────',
                        disabled: true
                    };
                    timeCodesIn = function(valuesString) {
                        var values;
                        values = valuesString ? valuesString.split(',') : [];
                        return values.map(function(value) {
                            return value.trim();
                        });
                    };
                    findTimesIn = (function(_this) {
                        return function(valuesString) {
                            var time, timeCodes, i, len, ref, ref1, results;
                            timeCodes = timeCodesIn(valuesString);
                            ref = _this.times;
                            results = [];
                            for (i = 0, len = ref.length; i < len; i++) {
                                time = ref[i];
                                if (ref1 = time.code, indexOf.call(timeCodes, ref1) >= 0) {
                                    results.push(time);
                                }
                            }
                            return results;
                    ...
                    removeTime = (function(_this) {
                        return function(time) {
                            return _this.times.splice(_this.times.indexOf(time), 1);
                        };
                    })(this);
                    includeOnlyRequestedTimes = (function(_this) {
                        return function() {
                            if (!$scope.only) {
                                return;
                            }
                            return _this.times = findTimesIn($scope.only);
                        };
                    })(this);
                    removeExcludedTimes = function() {
                        var time, i, len, ref, results;
                        if (!$scope.except) {
                            return;
                        }
                        ref = findTimesIn($scope.except);
                        results = [];
                        for (i = 0, len = ref.length; i < len; i++) {
                            time = ref[i];
                            results.push(removeTime(time));
                        }
                        return results;
                    };
                    updateWithPriorityTimes = (function(_this) {
                        return function() {
                            var i, len, priorityTimes, priorityTime, ref, results;
                            priorityTimes = findTimesIn($scope.priorities);
                            if (priorityTimes.length === 0) {
                                return;
                            }
                            _this.times.unshift(separator);
                            ref = priorityTimes.reverse();
                            results = [];
                            for (i = 0, len = ref.length; i < len; i++) {
                                priorityTime = ref[i];
                                removeTime(priorityTime);
                                results.push(_this.times.unshift(priorityTime));
                            }
                            return results;
                        };
                    })(this);
                    this.times = allTimes.slice();
                    includeOnlyRequestedTimes();
                    removeExcludedTimes();
                    updateWithPriorityTimes();
                    $scope.times = this.times;
